...
# You will need a bash utitlity intsalled, see https://gitforwindows.org/
...
targets = ["conference_call_management", "cognolink"]
#targets = ["conference_call_management"]
...
for targetSource in targets:
    #####
    sh_file = "{}{}{}_{}.sh".format(scriptFolder, shlocation, tableSh, targetSource) #JG
    tablelist_file = "{}{}tableList_{}.txt".format(scriptFolder, tableDump, targetSource) #JG
    ...
    sh_file = "{}{}{}_{}.sh".format(scriptFolder, shlocation, tablecolSh, targetSource) #JG
    command = '"C:\\Users\\jgreen\\AppData\\Local\\Programs\\Git\\bin\\sh.exe" --login -i -c "bash \\"' + sh_file + '\\""'
    command_args = shlex.split(command)
    process = Popen(command_args, stdout=PIPE, stderr=STDOUT, )
    output, err = process.communicate()
...
for enum, file in enumerate(os.listdir(scriptFolder+jsonDump)):
    ...
    config(
        ...
        schema          ='clean_{}'
            ...
            columnTxt.write("\n")
            columnTxt.write("{}-- meta\n".format(indent))
            columnTxt.write("{}r.meta_delivery_time{}as meta_delivery_time,\n".format(indent, " "*(charCountClean - len("r.meta_delivery_time"))))
            columnTxt.write("{}{{{{meta_process_time()}}}}{}as meta_process_time,\n".format(indent, " "*(charCountClean - 23)))
            columnTxt.write("{}'{}'{}as meta_source,\n".format(indent,datasetName, " "*(charCountClean - len("'"+datasetName+"'"))))
            columnTxt.write("{}1{}as meta_is_valid\n".format(indent, " "*(charCountClean - 1)))
...
#
        ...
        check_cols      = [\n""")
            ...
            for dict in data:
                ...
                if dict["type"].lower() == "timestamp":
                    ...
                    columnTxt.write("\n")
                    columnTxt.write("        '{}',\n".format(camel_to_snake(ColAliasTime)))
                    columnTxt.write("        '{}',\n".format(camel_to_snake(ColAliasDate)))
                    columnTxt.write("\n")

                else:
                    columnTxt.write("        '{}',\n".format(camel_to_snake(dict["name"])))
            columnTxt.write("""        ],
        ...
        partition_by    = {{'field': 'date(meta_process_time)',
            ...
            for dict in data:
                ...
                if dict["type"].lower() == "timestamp":
                    ...
                    columnTxt.write("\n")
                    columnTxt.write("{}c.{},\n".format(indent, camel_to_snake(ColAliasTime)))
                    columnTxt.write("{}c.{},\n".format(indent, camel_to_snake(ColAliasDate)))
                    columnTxt.write("\n")
                else:
                    columnTxt.write("{}c.{},\n".format(indent, camel_to_snake(dict["name"])))

            columnTxt.write("{}cast('2000-1-1' as date){}as create_date,\n".format(indent, " "*(charCountClean - len("cast('2000-1-1' as date)"))))
    ...
    config(
        ...
        cluster_by      = '{}_surrogate_key, {}_natural_key, meta_start_time'
            ...
            columnTxt.write("select\n")
            columnTxt.write("{}e.{}_surrogate_key,\n".format(indent, tableName))
            columnTxt.write("{}e.{}_natural_key,\n".format(indent, tableName))
            columnTxt.write("\n")
            columnTxt.write("{}-- attributes\n".format(indent))
            columnTxt.write("{}-- CHECK - REMOVE THE NATURAL KEY E.G. E.ID\n".format(indent))
            for dict in data:
