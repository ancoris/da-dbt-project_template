{{
    config(
        materialized    = 'scd2_events',
        schema          = 'pl_reference',
        ignore_deletes  = 'N',
        mode            = 'full',
        check_cols      = [
        'satisfaction_rating_id',
        'survey_question_id',

        '_fivetran_synced_time',
        '_fivetran_synced_date',

        'value',
        ],
        natural_key_col = 'satisfaction_rating_value_natural_key',
        modified_time   = 'meta_process_time',
        created_time    = 'create_date',
        cluster_by      = 'satisfaction_rating_value_natural_key',
        partition_by    = {'field': 'date(meta_process_time)',
                            'data_type':'date'}
    )
}}

select c.satisfaction_rating_id          as satisfaction_rating_value_natural_key,  /* CHECK THIS */
        c.satisfaction_rating_id,
        c.survey_question_id,

        c._fivetran_synced_time,
        c._fivetran_synced_date,

        c.value,
        cast('2000-1-1' as date)                                    as create_date,

        -- meta
        c.meta_source,
        c.meta_delivery_time,
        {{meta_process_time()}}                                     as meta_process_time
from {{ ref('satisfaction_rating_value_clean') }} c
where c.meta_is_valid = 1