{{
    config(
        ...
        cluster_by      = 'conversation_bcc_email_natural_key',
        partition_by    = {'field': 'date(meta_process_time)',
                            'data_type':'date'}
    )
}}

select c.conversation_id          as conversation_bcc_email_natural_key,  /* CHECK THIS */
        c.conversation_id,
        c.email,
...
from {{ ref('conversation_bcc_email_clean') }} c
